...
// Serve static files from current and parent directory (for flexible deployment)
app.use(express.static(__dirname));
app.use(express.static(path.join(__dirname, '..')));

// GET / - Redirect to index.html with fallback
app.get('/', (req, res) => {
    ...
    if (fs.existsSync(rootPath)) {
        ...
        res.status(404).send('ไม่พบไฟล์ index.html กรุณาตรวจสอบการอัปโหลดไฟล์');
    }
});
function loadDB() {
